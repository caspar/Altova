...
public class SV_RTFcompareSVRobot extends AbstractStyleVisionTest {
	...
	*/
	...
	// xbasefile[] contains result directory - we need base test directory here  ..\
	  int myIndex=xbasefile[i].substring(0,xbasefile[i].lastIndexOf("\\")).lastIndexOf("\\"); String testfile=xbasefile[i].substring(0,myIndex+1)+xdescription[i];
	  logger.fine("TCASE: "+xdescription[i]+" filename="+testfile);
	  robot.openFile(testfile);robot.delay(500);robot.setView(View.RTF);robot.delay(5000);
	...
	int i=8;
	  ...
	  robot.openFile(testfile);robot.delay(500);robot.setView(View.RTF);robot.delay(5000);delay(12000);
	  assertAreas(DIAGRAM_MAIN_WINDOW);
	 }
	@Test
